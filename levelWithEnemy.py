...
class enemy():
    def __init__(self,x,y,typ):
        ...
        self.ys=[y,y+1]
        self.type=typ
        dataenemy = ouverture("data/dataenemy.txt")
    ...
    def movement(self,counter,Ally,etagetest,number_same_type):
        if self.type==3:
            ...
            if self.y>1216:
                self.pos="-"
            self.y = round((75*cos(counter/(self.speedx*275)) + self.xs[0]*64),2)
            if self.pos=="+":
            
                self.x +=round(self.speedx/number_same_type,2)
            if self.pos=='-':
                self.x -=round(self.speedx/number_same_type,2)
...
def enemyAttack(xs,ys,pos,typ):
    if typ==1:
        ...
        return coupBasiqueEnnemis(xs,ys,pos,"attaque.png",("None"),10)               
                
def spellEnemy(xs,ys,direction,image,rang,effect,time):
    if direction=="+":
        l=[]
        for i in range(rang):
...
def coupBasiqueEnnemis(xs,ys,direction,image,effect,time):
    if direction=="+":
...
#end of the enemy class updated
...
class att:
    def __init__(self,time,value,effect):
        ...
        self.effect=effect
        self.intime=0
def damage(dmg,screen,Player,etagetest):
        if dmg!=[]:
            for i in dmg:
                for j in i.value[0]:
                        ...
                        for k in etagetest.niveaudata.enemy:
                            if player.xs[0]==j[0][1] or player.xs[1]==j[0][1]:
                                if player.ys[0]==j[0][0] or player.ys[1]==j[0][0]:
                                    i.hp-=Player.dgt
    ...
    def __init__(self,data,b):
        ...
        for i in range(len(data)):
            ...
            for j in range(len(data[i])):
                ...
                if self.datasalle[i][j][4:6]!="00":#implementation of enemies
                    self.enemy.append(enemy(i,j+-1,int(self.datasalle[i][j][4:6])))
                    self.number_enemy[int(self.datasalle[i][j][4:6])]+=1
...
def main(level,niv,screen,Ally):
    ...
    List_dangerous_zone = [] #this will be the list of active hitboxes from which the pkayer will take damage except contact damage
    while continu==1:
        ...
        if(etagetest.niveaudata.datasalle[Ally[0].ys[1]][Ally[0].xs[0]][:2]=="04" or etagetest.niveaudata.datasalle[Ally[0].ys[1]][Ally[0].xs[1]][:2]=="04"):
            ...
            Ally[0].liane=0
        if Ally[0].imo==0:                 
                if pygame.key.get_pressed()[pygame.K_z] and Ally[0].speedy==0 and Ally[0].jump==0:
                    Ally[0].speedy=-25
                    Ally[0].jump=1
        ...
        #debute of ennemy implemntation in the main
        ...
        for i in etagetest.niveaudata.enemy:
            ...
            if i.anim == None: # initialization of the i.anim which will count the frame of each individual enemies
                ...
                if i.type in [2,3]:
                    if i.anim/i.type == counter%i.type*10:
                        attack_temp,temp_effect,temp_time = enemyAttack( i.xs, i.ys, i.pos, i.type)
                        List_dangerous_zone.append([attack_temp,temp_effect,temp_time])

                elif i in [1,6]:
                    if i.anim/i.type == counter%i.type*10:
                        attack_temp1,attack_temp2,temp_effect,temp_time = enemyAttack( i.xs, i.ys, i.pos, i.typ)
                        List_dangerous_zone.append([attack_temp1,attack_temp2,temp_effect,temp_time])
                if i.anim<30:
                    if i.pos == '+':
                        screen.blit(i.imgimoret,(i.x,i.y))
                    else:
                        screen.blit(i.imgimo,(i.x,i.y))
                    i.anim += 1
                elif i.anim>=30:
                    if i.pos == '+':
                        screen.blit(i.img1ret,(i.x,i.y))
                    else:
                        screen.blit(i.img1,(i.x,i.y))
                    if i.anim == 60:
                        i.anim = 1
                    else:
                        i.anim+=1
        
        #update of the projectiles
        
        for i in range(len(List_dangerous_zone)):
            if len(List_dangerous_zone[i]) == 3:
                if List_dangerous_zone[i][0][-1] == '+':
                    screen.blit(pygame.transform.scale(pygame.image.load(List_dangerous_zone[i][0][1]),(30,30)),True,False),(List_dangerous_zone[i][0][0][0],List_dangerous_zone[i][0][0][1])
                else: 
                    screen.blit(pygame.transform.flip(pygame.transform.scale(pygame.image.load(List_dangerous_zone[i][0][1]),(30,30)),True,False),(List_dangerous_zone[i][0][0][0],List_dangerous_zone[i][0][0][1]))
                List_dangerous_zone[i].pop(0)
                if len(List_dangerous_zone[i])==0:
                    List_dangerous_zone.pop(i)
            else:       
                if List_dangerous_zone[i][0][-1] == '+':
                    screen.blit(pygame.transform.scale(pygame.image.load(List_dangerous_zone[i][0][2]),(30,30)),(List_dangerous_zone[i][0][1][0],List_dangerous_zone[i][0][1][1]))
                else:
                    screen.blit(pygame.transform.flip(pygame.transform.scale(pygame.image.load(List_dangerous_zone[i][0][2]),(30,30)),True,False),(List_dangerous_zone[i][0][1][0],List_dangerous_zone[i][0][1][1]))
                List_dangerous_zone.pop(i)
        
        for enemy in etagetest.niveaudata.enemy:
            for danger in etagetest.niveaudata.dmg:
                if enemy.xs == danger.value[0][0] and enemy.ys == danger.value[0][1]:
                    enemy.hp-=danger.time
                if enemy.hp <= 0:
                    enemy.state = False
      
        damage(etagetest.niveaudata.dmg,screen,Ally[0],etagetest)
