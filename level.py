...
def coupbasique(xs: list[int], ys: list[int], direction: str, image: str, effect: str, time: int) -> tuple[list[list[list[list[int], str, str]]], str, int]:
    if direction=="+":
...
def ouverture(dossier: str) -> tuple[list[list[str]], str | None]:
    r: str = open(dossier,"r")
    r: str = r.read()
    b: str = ""
    l: list[list[str]] = []
    a: list[str] = []
...
class salle():
    def __init__(self, data: list[list[str]], b: str) -> None:
        self.datasalle: list[list[str]] = data
        self.dataimg: list[list[pygame.Surface]] = self.datasalle
        self.background: pygame.Surface = pygame.image.load(b)
        dataimage: tuple[list[list[str]], str | None] = ouverture("Data/dataimage.txt")
        self.dataimg: list[list[pygame.Surface]] = []
        self.enemy: list[enemy] = []
        self.choice: list[list[pygame.Surface | list[int] | int]] = []
        self.choice2: list[int] = []
        
        for i in range(len(data)):
            l: list[pygame.Surface] = []
            for j in range(len(data[i])):
                if self.datasalle[i][j][4:6]!="00":
                      self.enemy.append(enemy(i,j,int(self.datasalle[i][j][4:6])))
                if self.datasalle[i][j][2:4]=="02":
                    self.inpose: list[int] = [i,j]
                if self.datasalle[i][j][2:4]=="04":
                    k: int = randint(1,6)
                    while k in self.choice2 or k==1:
                        k = randint(1,6)
                    data2: tuple[list[list[str]], str | None] = ouverture("Data/datapersonnage.txt")
...
def main(level,niv,screen,Ally):
    ...
    pygame.mixer.music.set_volume(0.0)  # Sets volume to 30% of maximum
    ...
    while continu==1:
        ...
        if Ally[0].imo==0:                 
                ...
                if pygame.key.get_pressed()[pygame.K_a]:
                     print("pressed 'a' but nothing happened")
